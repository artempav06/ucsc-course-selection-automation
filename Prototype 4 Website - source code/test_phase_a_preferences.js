...
function testAvoidedCoursesAreRemovedFromPreferenceDrivenChoicesWhenAlternativesExist() {
  const profile = makeProfile({
    ...
    avoidedCourses: ['ECON 135', 'ECON 136'],
    geConcentration: null
  });
  const courses = plannedCourses(Scheduler.generate(profile));
  assert(!courses.includes('ECON 135'), 'ECON 135 is avoidable and should not be scheduled when avoided');
  assert(!courses.includes('ECON 136'), 'ECON 136 is avoidable and should not be scheduled when avoided');
}
...
function testPhaseBAvailabilityScoringRanksEmptyQuartersBelowKnownOutOfWindowCourses() {
  withTemporaryCourses({
    'TEST EMPTY QUARTERS': {
      title: 'Test Empty Quarters', units: 5, quarters: [], division: 'upper', ge: null,
      concentrations: ['tim_entrepreneurship'], prereqs: [], rmpScore: 9
    },
    'TEST KNOWN FALL': {
      title: 'Test Known Fall', units: 5, quarters: ['F'], division: 'upper', ge: null,
      concentrations: ['tim_entrepreneurship'], prereqs: [], rmpScore: 0
    }
  }, () => {
    const profile = makeProfile({
      major: 'TIM_BS',
    ...
    const ranked = Scheduler.rankByConcentration(['TEST EMPTY QUARTERS', 'TEST KNOWN FALL'], 'tim_entrepreneurship', profile, new Set(), new Set());
    assert.strictEqual(
      ranked[0],
      'TEST KNOWN FALL',
