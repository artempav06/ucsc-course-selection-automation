...
test('avoided courses are not selected when alternatives exist', () => {
  const profile = makeProfile({ major: 'TIM_BS', concentration: 'tim_entrepreneurship', avoidedCourses: ['ECON 135', 'ECON 136'] });
  const courses = plannedCourses(Scheduler.generate(profile));
  assert(!courses.includes('ECON 135') && !courses.includes('ECON 136'), 'avoided TIM electives were selected despite alternatives');
});
...
test('official catalog alternatives are represented for audited non-RE majors', () => {
  ...
  assert(category('CSGD_BS', 'CGE_ELECTIVES').courses.includes('ECON 166A'), 'CSGD_BS missing ECON 166A cross-list elective');
  assert(category('CE_BS', 'DC').courses.includes('CSE 185E'), 'CE_BS missing CSE 185E DC option');
  assert(category('NDT_BS', 'DC').courses.includes('CSE 185E'), 'NDT_BS missing CSE 185E DC option');
