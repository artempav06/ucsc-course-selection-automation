...
function validateCourse(code, course, courses, errors, warnings, options) {
  ...
  if (course.concurrentPrereqs !== undefined) {
    if (!Array.isArray(course.concurrentPrereqs)) {
      add(errors, `${location}.concurrentPrereqs must be an array`);
    } else {
      course.concurrentPrereqs.forEach((group, groupIndex) => {
        if (!Array.isArray(group)) {
          add(errors, `${location}.concurrentPrereqs[${groupIndex}] must be an array`);
          return;
        }
        if (group.length === 0) add(errors, `${location}.concurrentPrereqs[${groupIndex}] must not be empty`);
        group.forEach((prereq, prereqIndex) => {
          if (typeof prereq !== 'string' || prereq.trim() === '') {
            add(errors, `${location}.concurrentPrereqs[${groupIndex}][${prereqIndex}] must be a non-empty string`);
          } else if (!courses[prereq]) {
            const message = `${location} references unknown concurrent prerequisite ${prereq}`;
            if (options.strictPrereqReferences) add(errors, message);
            else addWarning(warnings, message);
          }
        });
