...
function collectInterests() {
  const checks   = document.querySelectorAll(".interest-check:checked");
  const geChecks = document.querySelectorAll(".ge-interest-check:checked");
  AppState.profile.interests   = Array.from(checks).map(c => c.value);
  AppState.profile.geInterests = Array.from(geChecks).map(c => c.value);
}
  ...
  AppState.schedule.forEach((yearData, yearIdx) => {
    ...
    quarterKeys.forEach(q => {
      ...
      qHeader.innerHTML = `
        ...
        <span class="quarter-units">${isGapQuarter ? "GAP" : totalUnits + " units"}</span>
      `;
...
function createCourseCard(code, quarterKey, yearIdx) {
  ...
  const geBadge = course && course.ge
    ? `<span class="ge-badge">${course.ge}</span>`
    : "";
  ...
  card.innerHTML = `
    ...
    <div class="card-title">${course ? course.title : "Unknown Course"}</div>
